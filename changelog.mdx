...
- **[Improvement]**: Our `AnalyzeData` skill just got a brain boost! 🧠 We've fine-tuned the system prompt to be more specific about data analysis and added some guardrails to prevent data hallucination. It's like giving our skill a pair of data-focused glasses!
...
- **[Under the Hood]**: We've beefed up our `analyze_data.rb` file with more explicit instructions. It's like giving our skill a detailed map to navigate the data landscape. We've also improved the description of the `media_id` parameter - because clarity is key!

- **[Testing]**: Our QA game is stronger than ever! 💪 We've added a new test case in `analyze_data_eval.rb` to make sure our LLM returns an error when faced with empty data. We've also updated test cases in `analyze_data_eval.yml` to include the new system prompt about data validation. It's like giving our skill a practice run before the big show!
